...
export let QuizComponent = class QuizComponent {
    constructor(http, snackBar, router, dialog, sheet) {
        this.http = http;
        ...
        this.http
            ...
            next: (data) => {
                ...
                if (this.endTime.getTime() < currentTime.getTime()) {
                    this.snackBar.open('Waktu ujian telah habis. Terima kasih atas waktunya.', 'Tutup', {
                        duration: 1000,
                    });
    ...
    get progress() {
        const pending = this.questions.filter((question) => question.answer == null);
        return (((this.questions.length - pending.length) / this.questions.length) * 100 +
            '%');
    }
    openQuestion(index) {
        if (this.questions[index].type == 'drawing') {
            if (this.questions[index].files.length > 0) {
                this.snackBar.open('Anda sudah mengunggah jawaban untuk soal ini.', 'Tutup', {
                    duration: 1000,
                });
            }
            else {
    ...
    submit() {
        this.http
            .post(`${environment.apiURL}test/end`, {}, {
            headers: new HttpHeaders({
                'Content-Type': 'application/json',
                Authorization: 'Bearer ' + localStorage.getItem('authorization'),
                'Access-Control-Allow-Origin': '*',
            ...
            next: (_) => {
                ...
                this.isLoading = false;
                this.snackBar.open('Berhasil mengisi jawaban ujian. Terima kasih atas waktunya.', 'Tutup', {
                    duration: 1000,
                });
